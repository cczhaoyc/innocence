jQuery.validator.addMethod("isNotOracleTableReservedWord", function(value, element) {
	console.log(value);
...
// 验证数据库表名、字段名
jQuery.validator.addMethod("isTableColumnName", function(value, element) {
	if (value == null || value == undefined || value == "") {
		return true
	}
	var patrn = /^[A-Za-z]+[A-Za-z0-9_]*$/;
	if (!patrn.exec(value.replace(/[ ]/g, "")))
		return false
	return true
});
...
jQuery.validator.addMethod("validatePwd", function(value, element){
	...
	if (!patrn.exec(value.replace(/[ ]/g,""))) return false;
	$.ajax({
		url:"/system/userManager/validatePwd",
		data:{password:value},
		async:false,
		type:'post',
...
//excel
jQuery.validator.addMethod("isExcelNew", function(fileName, element){
	var suffix = fileName.substring(fileName.lastIndexOf(".")+1).toLowerCase();
